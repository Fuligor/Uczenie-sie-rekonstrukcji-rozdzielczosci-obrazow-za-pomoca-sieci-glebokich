...
from util import post_process_k
...
class KernelGAN:
    ...
    def __init__(self, conf):
        ...
        # Define the GAN
        self.G = networks.Generator(conf).to(self.conf.device)
        self.D = networks.Discriminator(conf).to(self.conf.device)
        ...
        # Input tensors
        self.g_input = torch.FloatTensor(1, 3, conf.input_crop_size, conf.input_crop_size).to(self.conf.device)
        self.d_input = torch.FloatTensor(1, 3, self.d_input_shape, self.d_input_shape).to(self.conf.device)

        # The kernel G is imitating
        self.curr_k = torch.FloatTensor(conf.G_kernel_size, conf.G_kernel_size).to(self.conf.device)

        # Losses
        self.GAN_loss_layer = loss.GANLoss(d_last_layer_size=self.d_output_shape, device=self.conf.device).to(self.conf.device)
        self.bicubic_loss = loss.DownScaleLoss(scale_factor=conf.scale_factor, device=self.conf.device).to(self.conf.device)
        self.sum2one_loss = loss.SumOfWeightsLoss().to(self.conf.device)
        self.boundaries_loss = loss.BoundariesLoss(k_size=conf.G_kernel_size, device=self.conf.device).to(self.conf.device)
        self.centralized_loss = loss.CentralizedLoss(k_size=conf.G_kernel_size, device=self.conf.device, scale_factor=conf.scale_factor).to(self.conf.device)
        self.sparse_loss = loss.SparsityLoss().to(self.conf.device)
        self.loss_bicubic = 0
    ...
    def calc_curr_k(self):
        ...
        delta = torch.Tensor([1.]).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).to(self.conf.device)
