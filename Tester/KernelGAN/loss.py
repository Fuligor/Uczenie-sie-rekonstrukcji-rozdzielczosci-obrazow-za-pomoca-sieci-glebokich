...
class GANLoss(nn.Module):
    ...
    def __init__(self, d_last_layer_size, device):
...
class CentralizedLoss(nn.Module):
    ...
    def __init__(self, k_size, device, scale_factor=.5):
...
class BoundariesLoss(nn.Module):
    ...
    def __init__(self, k_size, device):
        super(BoundariesLoss, self).__init__()
        self.mask = map2tensor(create_penalty_mask(k_size, 30, device), device)
