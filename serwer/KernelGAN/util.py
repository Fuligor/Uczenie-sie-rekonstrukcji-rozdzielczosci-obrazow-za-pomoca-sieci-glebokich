...
def map2tensor(gray_map, device):
...
def create_penalty_mask(k_size, penalty_scale, device):
    """Generate a mask of weights penalizing values close to the boundaries"""
    center_size = k_size // 2 + k_size % 2
    mask = create_gaussian(device=device, size=k_size, sigma1=k_size, is_tensor=False)
...
def create_gaussian(device, size, sigma1, sigma2=-1, is_tensor=False):
    """Return a Gaussian"""
