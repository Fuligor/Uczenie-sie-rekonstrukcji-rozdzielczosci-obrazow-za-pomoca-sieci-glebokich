...
def image_resize(img, scale=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False, ds_method='direct'):
    ...
    for i in range(out_H):
        ...
        for j in range(in_C):
            out_1[j, i, :] = np.matmul(img_aug[j, idx:idx + kernel_width, :].transpose(1, 0),(weights_H[i]))
    ...
    for i in range(out_W):
        ...
        for j in range(in_C):
            out_2[j, :, i] = np.matmul(out_1_aug[j, :, idx:idx + kernel_width],(weights_W[i]))
